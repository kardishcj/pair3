...
We then briefly mention the basics of what we're ultimately trying to do, and use this part to justify our exclusive use of the single ANES data source. We also use that opportunity to list the variables we will ultimately test, and include how they are coded in parentheses.

Then, we go into something like this:
...
age_df <- table(anes$dem_age_r_x) %>% as.data.frame() #creates a frequency table
#of the key age variable that's in the readalbe format of a data frame so we can get a sense of what the data looks like.

anes_genY <- subset(anes_dta, anes_dta$dem_age_r_x > 17 & anes_dta$dem_age_r_x < 33) #creates Millennial subset
anes_genX <-  subset(anes_dta, anes_dta$dem_age_r_x > 32 & anes_dta$dem_age_r_x < 48) #creates Generation X subset
anes_boomer <- subset(anes_dta, anes_dta$dem_age_r_x > 47 & anes_dta$dem_age_r_x < 67)
anes_silent <- subset(anes_dta, anes_dta$dem_age_r_x > 66 & anes_dta$dem_age_r_x < 88)
